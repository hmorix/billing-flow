...
export function drawCorporateCrimsonTemplate(params: PdfTemplateParams) {
  ...
  let y = 95;
  const cardW = 248;
  doc.roundedRect(margin, y, cardW, 80, 4).fill(lightBurg);
  doc.fillColor(burg).font('Helvetica-Bold').fontSize(7.5).text('BILLED FROM', margin + 10, y + 8);
  ...
  if (organization.tax_id) doc.fillColor('#374151').font('Helvetica-Bold').fontSize(7.5).text('GSTIN: ' + organization.tax_id, margin + 10, y + 58, { width: cardW - 20 });

  const toX = margin + cw - cardW;
  doc.roundedRect(toX, y, cardW, 80, 4).fill(lightBurg);
  doc.fillColor(burg).font('Helvetica-Bold').fontSize(7.5).text('BILLED TO', toX + 10, y + 8);
  ...
  if (client.tax_id) doc.fillColor('#374151').font('Helvetica-Bold').fontSize(7.5).text('GSTIN: ' + client.tax_id, toX + 10, y + 58, { width: cardW - 20 });

  y += 90;
  // Table
  doc.rect(margin, y, cw, 22).fill(burg);
  doc.fillColor('#fff').font('Helvetica-Bold').fontSize(8);
  doc.text('#', margin + 6, y + 7, { width: 20 });
  doc.text('Description', margin + 30, y + 7, { width: 220 });
  ...
  items.forEach((item: any, i: number) => {
    ...
    if (y + rowH > 620) { doc.addPage(); doc.rect(margin, 40, cw, 22).fill(burg); doc.fillColor('#fff').font('Helvetica-Bold').fontSize(8); doc.text('#', margin + 6, 47, { width: 20 }); doc.text('Description', margin + 30, 47, { width: 220 }); doc.text('Unit Price', margin + 255, 47, { width: 80, align: 'right' }); doc.text('Qty', margin + 340, 47, { width: 45, align: 'center' }); doc.text('Amount', margin + 390, 47, { width: 115, align: 'right' }); y = 62; }
    if (i % 2 === 1) doc.rect(margin, y, cw, rowH).fill(lightBurg);
    doc.fillColor('#9ca3af').font('Helvetica').fontSize(8).text(String(i + 1), margin + 6, y + 6, { width: 20 });
    doc.fillColor('#1f2937').text(itemDesc, margin + 30, y + 6, { width: 220 });
    if (metaSubtitle) {
      doc.fillColor(burg).font('Helvetica-Bold').fontSize(6.8).text(metaSubtitle, margin + 30, y + 6 + descH + 1, { width: 220 });
    }
    doc.fillColor('#4b5563').font('Helvetica').text(formatCurrency(item.unit_price, invoice.currency), margin + 255, y + 6, { width: 80, align: 'right' });
  ...
  doc.roundedRect(margin, y, 255, 48, 4).fill(lightBurg);
  doc.fillColor(burg).font('Helvetica-Bold').fontSize(7.5).text('AMOUNT IN WORDS', margin + 8, y + 6);
  ...
  doc.fillColor('#111827').text(formatCurrency(taxInfo.subtotal, invoice.currency), calcX + calcW - 105, y, { width: 105, align: 'right' }); y += 14;
  if (taxInfo.discount > 0) { doc.fillColor('#4b5563').text('DISCOUNT :', calcX, y, { width: calcW - 110, align: 'right' }); doc.fillColor('#ef4444').text('-' + formatCurrency(taxInfo.discount, invoice.currency), calcX + calcW - 105, y, { width: 105, align: 'right' }); y += 14; }
  if (taxInfo.hasCgstSgst) {
    doc.fillColor('#4b5563').text('CGST (' + taxInfo.cgstRate + '%) :', calcX, y, { width: calcW - 110, align: 'right' }); doc.fillColor('#111827').text(formatCurrency(taxInfo.cgstAmount, invoice.currency), calcX + calcW - 105, y, { width: 105, align: 'right' }); y += 14;
    doc.fillColor('#4b5563').text('SGST (' + taxInfo.sgstRate + '%) :', calcX, y, { width: calcW - 110, align: 'right' }); doc.fillColor('#111827').text(formatCurrency(taxInfo.sgstAmount, invoice.currency), calcX + calcW - 105, y, { width: 105, align: 'right' }); y += 14;
  } else if (taxInfo.hasIgst) {
    doc.fillColor('#4b5563').text('IGST (' + taxInfo.igstRate + '%) :', calcX, y, { width: calcW - 110, align: 'right' }); doc.fillColor('#111827').text(formatCurrency(taxInfo.igstAmount, invoice.currency), calcX + calcW - 105, y, { width: 105, align: 'right' }); y += 14;
  } else if (taxInfo.hasFlatTax) {
    doc.fillColor('#4b5563').text('TAX (' + taxInfo.taxRate + '%) :', calcX, y, { width: calcW - 110, align: 'right' }); doc.fillColor('#111827').text(formatCurrency(taxInfo.taxAmount, invoice.currency), calcX + calcW - 105, y, { width: 105, align: 'right' }); y += 14;
  } else if (taxInfo.hasItemTax) {
    doc.fillColor('#4b5563').text('GST TAX (ITEM-WISE) :', calcX, y, { width: calcW - 110, align: 'right' }); doc.fillColor('#111827').text(formatCurrency(taxInfo.taxAmount, invoice.currency), calcX + calcW - 105, y, { width: 105, align: 'right' }); y += 14;
  }
  doc.roundedRect(calcX, y, calcW, 24, 4).fill(burg);
  doc.fillColor('#fff').fontSize(9.5).font('Helvetica-Bold').text('TOTAL DUE :', calcX + 8, y + 7, { width: 100, align: 'left' });
