...
export function drawGoldenEleganceTemplate(params: PdfTemplateParams) {
  ...
  doc.roundedRect(margin, cardStartY, cardW, cardH, 4).fill(warmBg);
  doc.fillColor(gold).font('Helvetica-Bold').fontSize(7.5).text('BILLED FROM', margin + 10, cardStartY + 8);
  doc.fillColor('#1c1917').font('Helvetica-Bold').fontSize(9).text(organization.name, margin + 10, cardStartY + 20, { width: cardW - 20 });
  let fromY = cardStartY + 20 + fromNameH + 2;
  if (organization.address) { doc.fillColor('#78350f').font('Helvetica').fontSize(7.5).text(organization.address, margin + 10, fromY, { width: cardW - 20 }); fromY += fromAddrH + 2; }
  if (organization.tax_id) { doc.fillColor('#1c1917').font('Helvetica-Bold').fontSize(7.5).text('GSTIN: ' + organization.tax_id, margin + 10, fromY, { width: cardW - 20 }); }

  const toX = margin + cw - cardW;
  doc.roundedRect(toX, cardStartY, cardW, cardH, 4).fill(warmBg);
  doc.fillColor(gold).font('Helvetica-Bold').fontSize(7.5).text('BILLED TO', toX + 10, cardStartY + 8);
  doc.fillColor('#1c1917').font('Helvetica-Bold').fontSize(9).text(client.name, toX + 10, cardStartY + 20, { width: cardW - 20 });
  let toY = cardStartY + 20 + toNameH + 2;
  if (client.company_name) { doc.fillColor('#78350f').font('Helvetica-Bold').fontSize(8).text(client.company_name, toX + 10, toY, { width: cardW - 20 }); toY += toCompH + 2; }
  if (client.address) { doc.fillColor('#78350f').font('Helvetica').fontSize(7.5).text(client.address, toX + 10, toY, { width: cardW - 20 }); toY += toAddrH + 2; }
  if (client.tax_id) { doc.fillColor('#1c1917').font('Helvetica-Bold').fontSize(7.5).text('GSTIN: ' + client.tax_id, toX + 10, toY, { width: cardW - 20 }); }

  let y = cardStartY + cardH + 12;

  const renderTableHeader = (headerY: number) => {
    doc.rect(margin, headerY, cw, 22).fill(gold);
    doc.fillColor('#fff').font('Helvetica-Bold').fontSize(8);
    doc.text('#', margin + 6, headerY + 7, { width: 20 }); doc.text('Item Description', margin + 30, headerY + 7, { width: 220 });
  ...
  items.forEach((item: any, i: number) => {
    ...
    if (y + rowH > 620) { doc.addPage(); doc.rect(0, 0, 595, 842).fill(ivory); y = renderTableHeader(40); }
    if (i % 2 === 1) doc.rect(margin, y, cw, rowH).fill(warmBg);
    const textY = y + 6;
    doc.fillColor('#a16207').font('Helvetica').fontSize(8).text(String(i + 1), margin + 6, textY, { width: 20 });
    doc.fillColor('#1c1917').font('Helvetica').fontSize(8).text(itemDesc, margin + 30, textY, { width: 220, lineGap: 1 });
    if (metaSubtitle) {
      const subY = textY + descH + 3;
      doc.fillColor(gold).font('Helvetica-Bold').fontSize(6.8).text(metaSubtitle, margin + 30, subY, { width: 220, lineGap: 1 });
    }
    doc.fillColor('#78350f').font('Helvetica').fontSize(8).text(formatCurrency(item.unit_price, invoice.currency), margin + 255, textY, { width: 80, align: 'right' });
    doc.fillColor('#78350f').font('Helvetica').fontSize(8).text(Number(item.quantity).toFixed(0), margin + 340, textY, { width: 45, align: 'center' });
    doc.fillColor('#1c1917').font('Helvetica-Bold').fontSize(8).text(formatCurrency(itemTotal, invoice.currency), margin + 390, textY, { width: 115, align: 'right' });
    doc.font('Helvetica').fontSize(8);
    doc.strokeColor('#fde68a').lineWidth(0.8).moveTo(margin, y + rowH).lineTo(margin + cw, y + rowH).stroke();
    y += rowH;
  });
  ...
  doc.roundedRect(margin, summaryStartY, 255, wordsH, 4).fill(warmBg);
  doc.fillColor(gold).font('Helvetica-Bold').fontSize(7.5).text('AMOUNT IN WORDS', margin + 8, summaryStartY + 6);
  doc.fillColor('#1c1917').font('Helvetica-Oblique').fontSize(7.5).text(words, margin + 8, summaryStartY + 18, { width: 240, lineGap: 1 });

  let calcY = summaryStartY;
  doc.fillColor('#78350f').font('Helvetica-Bold').fontSize(8);
  doc.text('SUBTOTAL :', calcX, calcY, { width: calcW - 110, align: 'right' }); doc.fillColor('#1c1917').text(formatCurrency(taxInfo.subtotal, invoice.currency), calcX + calcW - 105, calcY, { width: 105, align: 'right' }); calcY += 14;
  if (taxInfo.discount > 0) { doc.fillColor('#78350f').text('DISCOUNT :', calcX, calcY, { width: calcW - 110, align: 'right' }); doc.fillColor('#dc2626').text('-' + formatCurrency(taxInfo.discount, invoice.currency), calcX + calcW - 105, calcY, { width: 105, align: 'right' }); calcY += 14; }
  if (taxInfo.hasCgstSgst) { doc.fillColor('#78350f').text('CGST (' + taxInfo.cgstRate + '%) :', calcX, calcY, { width: calcW - 110, align: 'right' }); doc.fillColor('#1c1917').text(formatCurrency(taxInfo.cgstAmount, invoice.currency), calcX + calcW - 105, calcY, { width: 105, align: 'right' }); calcY += 14; doc.fillColor('#78350f').text('SGST (' + taxInfo.sgstRate + '%) :', calcX, calcY, { width: calcW - 110, align: 'right' }); doc.fillColor('#1c1917').text(formatCurrency(taxInfo.sgstAmount, invoice.currency), calcX + calcW - 105, calcY, { width: 105, align: 'right' }); calcY += 14; }
  else if (taxInfo.hasIgst) { doc.fillColor('#78350f').text('IGST (' + taxInfo.igstRate + '%) :', calcX, calcY, { width: calcW - 110, align: 'right' }); doc.fillColor('#1c1917').text(formatCurrency(taxInfo.igstAmount, invoice.currency), calcX + calcW - 105, calcY, { width: 105, align: 'right' }); calcY += 14; }
  else if (taxInfo.hasFlatTax) { doc.fillColor('#78350f').text('TAX (' + taxInfo.taxRate + '%) :', calcX, calcY, { width: calcW - 110, align: 'right' }); doc.fillColor('#1c1917').text(formatCurrency(taxInfo.taxAmount, invoice.currency), calcX + calcW - 105, calcY, { width: 105, align: 'right' }); calcY += 14; }
  else if (taxInfo.hasItemTax) { doc.fillColor('#78350f').text('GST TAX (ITEM-WISE) :', calcX, calcY, { width: calcW - 110, align: 'right' }); doc.fillColor('#1c1917').text(formatCurrency(taxInfo.taxAmount, invoice.currency), calcX + calcW - 105, calcY, { width: 105, align: 'right' }); calcY += 14; }
  doc.roundedRect(calcX, calcY, calcW, 24, 4).fill(gold);
