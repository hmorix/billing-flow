...
export function drawGoldenEleganceTemplate(params: PdfTemplateParams) {
  ...
  items.forEach((item: any, i: number) => {
    ...
    doc.fillColor('#1c1917').text(itemDesc, margin + 30, y + 6, { width: 220 });
    if (metaSubtitle) {
      doc.fillColor(gold).font('Helvetica-Bold').fontSize(6.8).text(metaSubtitle, margin + 30, y + 6 + descH + 1, { width: 220 });
    }
    doc.fillColor('#78350f').font('Helvetica').text(formatCurrency(item.unit_price, invoice.currency), margin + 255, y + 6, { width: 80, align: 'right' });
  ...
  else if (taxInfo.hasItemTax) { doc.fillColor('#78350f').text('GST TAX (ITEM-WISE) :', calcX, y, { width: calcW - 110, align: 'right' }); doc.fillColor('#1c1917').text(formatCurrency(taxInfo.taxAmount, invoice.currency), calcX + calcW - 105, y, { width: 105, align: 'right' }); y += 14; }
