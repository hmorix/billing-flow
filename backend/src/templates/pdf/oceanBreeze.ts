...
export function drawOceanBreezeTemplate(params: PdfTemplateParams) {
  ...
  items.forEach((item: any, i: number) => {
    ...
    if (metaSubtitle) {
      doc.fillColor('#1f2937').font('Helvetica-Bold').fontSize(6.8).text(metaSubtitle, margin + 30, y + 6 + descH + 1, { width: 220 });
    }
    doc.font('Helvetica');
    ...
    doc.font('Helvetica'); doc.strokeColor('#bae6fd').lineWidth(0.8).moveTo(margin, y + rowH).lineTo(margin + cw, y + rowH).stroke(); y += rowH;
  });
  ...
  doc.roundedRect(margin, y, 255, 48, 4).fill(sky);
  doc.fillColor(azure).font('Helvetica-Bold').fontSize(7.5).text('AMOUNT IN WORDS', margin + 8, y + 6);
  ...
  doc.roundedRect(calcX, y, calcW, 24, 4).fill(azure);
