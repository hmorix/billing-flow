...
export function drawOceanBreezeTemplate(params: PdfTemplateParams) {
  ...
  doc.roundedRect(margin, cardStartY, cardW, cardH, 4).fill(sky);
  doc.fillColor(azure).font('Helvetica-Bold').fontSize(7.5).text('BILLED FROM', margin + 10, cardStartY + 8);
  ...
  if (organization.address) { doc.fillColor('#4b5563').font('Helvetica').fontSize(7.5).text(organization.address, margin + 10, fromY, { width: cardW - 20 }); fromY += fromAddrH + 2; }
  if (organization.tax_id) { doc.fillColor('#374151').font('Helvetica-Bold').fontSize(7.5).text('GSTIN: ' + organization.tax_id, margin + 10, fromY, { width: cardW - 20 }); }

  const toX = margin + cw - cardW;
  doc.roundedRect(toX, cardStartY, cardW, cardH, 4).fill(sky);
  doc.fillColor(azure).font('Helvetica-Bold').fontSize(7.5).text('BILLED TO', toX + 10, cardStartY + 8);
  ...
  if (client.company_name) { doc.fillColor('#374151').font('Helvetica-Bold').fontSize(8).text(client.company_name, toX + 10, toY, { width: cardW - 20 }); toY += toCompH + 2; }
  if (client.address) { doc.fillColor('#4b5563').font('Helvetica').fontSize(7.5).text(client.address, toX + 10, toY, { width: cardW - 20 }); toY += toAddrH + 2; }
  if (client.tax_id) { doc.fillColor('#374151').font('Helvetica-Bold').fontSize(7.5).text('GSTIN: ' + client.tax_id, toX + 10, toY, { width: cardW - 20 }); }

  let y = cardStartY + cardH + 12;

  const renderTableHeader = (headerY: number) => {
    doc.rect(margin, headerY, cw, 22).fill(azure);
